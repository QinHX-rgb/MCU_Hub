...
#define I2C_MAX_CONSECUTIVE_FAULTS (3)

/* OLED 断线后快速熔断，避免每个显示字节都等待超时而拖住控制循环。 */
static uint8_t g_oled_i2c_faults = 0;
static uint8_t g_oled_available = 1;
...
void OLED_WR_Byte(uint8_t dat,uint8_t mode)
{
    ...
    /* OLED 断线或 I2C 总线异常时，控制器可能一直不进入 IDLE。
     * 原来的无限等待会卡死整个主循环；现在超时后恢复总线并放弃本字节。 */
    while (!(DL_I2C_getControllerStatus(I2C_OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE))
    {
        ...
        {
            oled_i2c_sda_unlock();
            if (++g_oled_i2c_faults >= I2C_MAX_CONSECUTIVE_FAULTS)
                g_oled_available = 0;
            return;
        }
    }
