...
int main(void)
{
    ...
    while (!DL_GPIO_readPins(GPIO_BUTTON_PIN_BUTTON_PORT,
        ...
        /* Key1: 检测高电平 → 消抖 → 确认 → 等松手 */
        if (DL_GPIO_readPins(GPIO_BUTTON_PIN_Key1_PORT,
                             GPIO_BUTTON_PIN_Key1_PIN)) {
            delay_ms(20);
            if (DL_GPIO_readPins(GPIO_BUTTON_PIN_Key1_PORT,
                                 GPIO_BUTTON_PIN_Key1_PIN)) {
                Motor_Control_SetProfile(0);
                profile = 0;
                Buzzer_Beep();
                while (DL_GPIO_readPins(GPIO_BUTTON_PIN_Key1_PORT,
                                        GPIO_BUTTON_PIN_Key1_PIN));
            }
        }

        /* Key2: 检测高电平 → 消抖 → 确认 → 等松手 */
        if (DL_GPIO_readPins(GPIO_BUTTON_PIN_Key2_PORT,
                             GPIO_BUTTON_PIN_Key2_PIN)) {
            delay_ms(20);
            if (DL_GPIO_readPins(GPIO_BUTTON_PIN_Key2_PORT,
                                 GPIO_BUTTON_PIN_Key2_PIN)) {
                Motor_Control_SetProfile(1);
                profile = 1;
                Buzzer_Beep(); delay_ms(100); Buzzer_Beep();
                while (DL_GPIO_readPins(GPIO_BUTTON_PIN_Key2_PORT,
                                        GPIO_BUTTON_PIN_Key2_PIN));
            }
        }

        /* 第 1 行: 启动提示 + 方案 */
        snprintf(buf, sizeof(buf), "S1:Go  P=%d", profile);
    ...
    while (1)
    {
        ...
        if (Flag == 4) {                        // 正常循线
            if (beeped) { Buzzer_Beep(); beeped = 0; }
            Control_Line_Track();
